...
const findFunc = /const requestPermissionsAndEnumerate = async \(\) => \{[\s\S]*?checkVolume\(\);\r?\n\r?\n      \} catch \(err: any\) \{[\s\S]*?setPermissionsGranted\(false\);\r?\n      \}\r?\n    \};/;

const replaceFunc = `const requestPermissionsAndEnumerate = async () => {
      let stream = null;
      try {
...
const findUseEffect = `    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedVideoId, selectedAudioId]);`;

const replaceUseEffect = `    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);`;

content = content.replace(findUseEffect, replaceUseEffect);

content = content.replace("Waiting for permissions...", "No devices available (or awaiting permissions)");
content = content.replace("Waiting for permissions...", "No devices available (or awaiting permissions)");
